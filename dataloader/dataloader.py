...
class CustomDataLoader(BaseDataLoader):
    def __init__(self, data_path: Dict[str, any]):
        ...
        self.image_label_df: pd.DataFrame = pd.read_csv(self.data_path['image_labels_path']).drop(
    ...
    def _get_splits(self, image_label_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        train, temp_test = train_test_split(image_label_df, train_size=0.8,
    ...
    def get_train_dataloader(self, batch_size: int = 32):
        self._train_dataset = CustomDataset(image_data_path=self.data_path['image_path'],
                                            image_label_df=self.train,
                                            is_train=True,
                                            transform=DataTransform(input_size=self.data_path['image_size'])
                                            )

        self.train_dataloader = DataLoader(self._train_dataset, batch_size=batch_size, shuffle=True)
        return self.train_dataloader

    def get_validation_dataloader(self, batch_size: int = 32):
        self._validation_dataset = CustomDataset(image_data_path=self.data_path['image_path'],
                                                 image_label_df=self.validation,
                                                 is_train=False,
                                                 transform=DataTransform(input_size=self.data_path['image_size']))

        self.validation_dataloader = DataLoader(self._validation_dataset, batch_size=batch_size)
        return self.validation_dataloader

    def get_test_dataloader(self, batch_size: int = 32):
        self._test_dataset = CustomDataset(image_data_path=self.data_path['image_path'],
